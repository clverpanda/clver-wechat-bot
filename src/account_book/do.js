...
const AccountBookDo = async (msg, config) => {
    if (!config.account_book) return;
    const contact = msg.from();
    const text = msg.text();
    const room = msg.room();
    if (room) {
        const topic = await room.topic();
        if (!config.account_book_contact.includes(topic)) return;
        ...
        if (msg.self()) {
            if (!config.account_book_contact.includes(toContact.name())) return;
            ...
            if (!config.account_book_contact.includes(name)) return;
